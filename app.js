...
const app = createApp({
  ...
  setup() {
    ...
    function execTerm() {
      ...
      else if (b==='version')           addOut('ALONE CODE STUDIO v10 | Offline | Vue3+CM6','text-line');
      ...
      else if (b==='help')              addOut('Commands: run clear ls echo pwd version date status import download find rename new font wrap theme help','info');
    ...
    async function sendAI() {
      ...
      try {
        ...
        const res=await fetch(ANTHROPIC_URL,{method:'POST',headers,body:JSON.stringify({model:'claude-sonnet-4-20250514',max_tokens:2048,system:`Expert coding assistant in ALONE CODE STUDIO v10. All 20 languages have offline simulators. Be concise, use markdown code blocks. Files: ${flatFiles.value.map(f=>f.name).join(', ')}.`,messages:msgs})});
        const data=await res.json();
    ...
    document.addEventListener('keydown', e => {
      ...
      if (e.key==='Escape')        { findOpen.value=false; settingsOpen.value=false; importOpen.value=false; renameOpen.value=false; }
    });
